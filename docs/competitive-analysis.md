...
|---|---|---|---|:---:|
| G1 | **Use the OSC 133 marks we already emit** | 1.3.0 | `internal/shellinit/` injects OSC 133 A/B/D into every shell and the daemon reads only `D` (`detectOSC133Exit`). Ghostty turns the same marks into jump-to-prompt, select-a-command's-whole-output, click-to-move-cursor inside the prompt, and "don't confirm close while the cursor sits at a prompt". We already pay the cost and take almost none of the value. Cheapest large win on this page. | M |
...
| G4 | **A duration floor on command-finish alerts** | 1.3.0 | `notify-on-command-finish-after` fires only when the command ran longer than N. Quil's `detectOSC133Exit` emits a `command_complete` for every command however short, so a one-second command is as loud as a ten-minute one. Needs the `B` (command-start) mark timestamped per pane — which is G1's work — plus one config value. | S |
